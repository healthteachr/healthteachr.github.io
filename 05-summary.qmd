...
To load an `.rds` file, you can use the `readRDS()` function. To ensure that you get the same output as the rest of this chapter, you can download the dataset <a href="data/ncod_tweets.rds" download>ncod_tweets.rds</a> and load it using this function.
...
```{r eval = FALSE}
tweet_summary2 <- tweets %>% # start with the object tweets and then
