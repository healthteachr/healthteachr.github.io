...
`r glossary("project", "Projects")` in RStudio are a way to group all
the files you need for one project. Most projects include
`r glossary("script", "scripts")`, data files, and output files like the
PDF report created by the script or images.

### Default working directory

First, make a new `r glossary("directory")` (i.e., folder) on your
...
-   file and directory names should only contain letters, numbers,
    ...
    name and `r glossary("extension")` (that means no spaces!)
...
-   `r path("Data (Athletes) 11-15.xls")`
-   `r path("Athletes Data Nov 12.xls")`
-   `r path("final report2.doc")`
-   `r path("project notes.txt")`
-   `r path("Rugby Data November 15.xls")`
...
-   `r path("data_athletes_2021-11-12.xls")`
-   `r path("data_athletes_2021-11-15.xls")`
-   `r mcq(c("rugby-data_2021-11-15.xls", "data-rugby-2021_11_15.xls", answer = "data_rugby_2021-11-15.xls", "data_2021-11-15_rugby.xls"))`
...
Throughout this course we will use `r glossary("R Markdown")` to create
reproducible reports with a table of contents, text, tables, images, and
code. The text can be written using `r glossary("markdown")`, which is a
...
(if you are on a Mac and can see the file `r glossary("extension")`,
...
Now we are going to `r glossary("knit")`, or compile, the file into a
...
> My name is `r name` and I am `r age` years old. It is
> `r christmas - today` days until Christmas, which is my favourite
> holiday.
...
```{r, message=FALSE, filename="hockey_data.Rmd"}
...
> Error in
> read_csv("https://healthteachr.github.io/ads-v2/data/nhldraft.csv") :\
...
`r glossary("working directory")`, and we have already set up the main
directory `r path("02-reports")`for this chapter.
...
`r path("data")` and saving any image files to a subdirectory called
`r path("images")`. Using subdirectories helps avoid one single folder
becoming too cluttered, which is important if you're working on big
projects.

In your `r path("02-reports")` directory, create a new folder named
`r path("data")`, [download a copy of the hockey data
file](https://healthteachr.github.io/ads-v2/data/nhldraft.csv){download=""},
...
```{r read-csv, eval = FALSE, filename="baseball_data.Rmd"}
...
We're going to write a basic report for this baseball dataset using R
...
```{r hockey_counts, filename="hockey_data.Rmd"}
...
```{r hockey_counts_show, echo = FALSE}
hockey_counts
```
...
You can use the visual `r glossary("markdown")` editor if you have
...
```{verbatim, lang="md", filename="hockey_data.Rmd"}
## Hockey report
...
```{r fig-example, eval = FALSE, filename="hockey_data.Rmd"}
...
```{r, eval = FALSE, filename="hockey_data.Rmd"}
hockey_counts
```
...
```{r knitr-setup2, eval=FALSE, verbatim="r setup, include=FALSE", filename="hockey_data.Rmd"}
...
```{r show-code, eval=FALSE, verbatim = "r, echo = TRUE", filename="hockey_data.Rmd"}
ggplot(data = hockey_counts, 
       mapping = aes(x = position, 
                     y = n, 
                     fill = position)) +
...
```{r change-image2, eval=FALSE, verbatim = "r, fig.width = 10, fig.height = 20", filename="hockey_data.Rmd"}
ggplot(data = hockey_counts, 
...
Finally, the `r glossary("YAML")` header is the bit at the very top of
...
`r hide()` --- title: "My Job" author: "Me" output: html_document:
df_print: kable theme: version: 4 bootswatch: sandstone --- `r unhide()`
...
```{r, webex.hide= TRUE}
knitr::include_graphics("https://healthteachr.github.io/ads-v2/images/logos/logo.png")
```
...
```         
![Applied Data Skills](https://healthteachr.github.io/ads-v2/images/logos/logo.png)
```
...
sentence: "This report was created using `r R.version.string`." You can
...
`r hide()` Click on the knit button or run the following code in the
